...
| EVAL-004 | 修改刪除、匯出或解除配對 | 依 [W14 accepted lifecycle／export 契約](../docs/architecture/03-w14-data-lifecycle-contract.md)處理雙方資料；加入失敗、部分完成與第三人隔離情境；要求人類 review | 自行猜測共同資料所有權；只驗證單方畫面 |
...
| EVAL-006 | 尚未被 accepted SSOT 覆蓋的帳號、同步、聊天、照片或推播方案 | 明確標示 provisional；提出最小兩手機／兩 Apple ID 驗證；更新決策文件後才落實 | 把未實測方案寫成已確定架構或完成項目；覆寫 W14 已接受的帳號刪除／匯出語意 |
