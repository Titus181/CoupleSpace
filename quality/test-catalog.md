...
| ARCHIVE-001 | W1 封存／匯出 | 單方越權、照片缺失、容量與 staging 清理錯誤；不得把舊候選當作 W14 export v1 完成證據 | pgTAP／Unit＋真機 | `relationship_archive.test.sql`、`archived_photo_access.test.sql`、`CoupleSpaceTests/CoupleSpaceTests.swift`、`manual/deletion-and-unpairing.md`、`docs/architecture/03-w14-data-lifecycle-contract.md` | A、B、D |
| DELETE-001 | W1／切片 7 刪除／解除配對 | 單方刪除影響另一方、GC／雙方結果不一致、正式入口繞過待送內容、begin 成功後誤回 active、重複封存、closing／archived 未收斂，或 Release 露出 W1 低階工具；不得取代 W14 帳號刪除契約 | pgTAP／Unit／UI＋真機 | `personal_archive_deletion_queue.test.sql`、`CoupleSpaceTests/AppSkeletonTests.swift`、`CoupleSpaceUITests/CoupleSpaceUITests.swift`、`manual/deletion-and-unpairing.md`、`manual/two-iphone.md`、`docs/architecture/03-w14-data-lifecycle-contract.md` | A、B、D |
| W14-CONTRACT-001 | W14-01 資料生命週期契約 | 任一資料類型缺少 actor／可見性／保存／封存／匯出／刪除／tombstone 結果，帳號刪除或 export schema 未標 accepted，audit 保存私人正文，A／B／C fixture 缺少第三人隔離，或 W13 基線被誤寫成 W14 runtime 完成 | Document review／Harness | `docs/architecture/03-w14-data-lifecycle-contract.md`、`docs/architecture/01-disaster-recovery.md`、`docs/decisions/product-decisions.md`、`docs/decisions/technical-decisions.md`、`docs/ARCHITECTURE.md`、`docs/operations/01-v1-development-roadmap.md`、`docs/product/04-iphone-mvp-scope.md`、`quality/test-catalog.md`、`evals/README.md` | W14-01 文件 gate（links／Harness／diff hygiene；不套用 Gate A／B runtime 要求） |
...
| MOMENT-005 | W14-03 Moment 永久清除／tombstone／最後引用 GC | 未滿 30 天或未經 server authority 即 purge、期限後仍可復原、重複 sweep／worker retry 重複寫 tombstone 或 enqueue、收藏來源正文／約定／照片遭連帶刪除、direct photo 最後引用消失前 object 被清除、worker claim／失敗不可重試或稽核、tombstone 含正文，或舊資料／cache 使已永久刪除內容復活 | pgTAP／Unit／Function＋故障注入＋雙隔離 Simulator | `moment_permanent_purge.test.sql`、`supabase/functions/process-storage-gc/index.test.ts`、`CoupleSpaceTests/AppSkeletonTests.swift` | A、B；local server／Function 故障注入與雙 Simulator，本切片不跑真機；正式 restore replay 留到 G15／G17 |
...
`W14-CONTRACT-001` 只關閉 W14-01 的文件契約完整性，不宣稱帳號刪除、export v1、完整封存、audit／tombstone migration 或 runtime 已存在。W14 後續每個實作切片須在加入實際測試位置時分配不重複的 runtime ID；W14-02 的 runtime 責任由 `MOMENT-004` 承接，W14-03 的永久清除、tombstone 與最後引用 GC 由 `MOMENT-005` 承接，W14-04～08 詳細分工目前為 `未記錄`，不得先填虛構測試或 PASS。`MOMENT-005` 的本切片 gate 不含真機，也不把 local tombstone／故障注入當作正式 restore replay；後者仍由 G15／G17 關閉。集中雙真機複驗只在 W14 最後整合切片執行。
