...
| APPOINTMENT-001 | W11 基本共同約定 | 非伴侶讀寫、Outbox／ack 遺失重試建立重複約定、來源訊息產生重複卡片、長按未確認便建立、取消後被編輯復活、提醒晚於開始時間，或專屬討論跨約定／主對話串線、未讀游標混用、離線文字遺失／重複 | pgTAP／Unit／UI＋真機 | `shared_appointments.test.sql`、`appointment_discussions.test.sql`、`CoupleSpaceTests/AppSkeletonTests.swift`、`CoupleSpaceUITests/CoupleSpaceUITests.swift`、`manual/two-iphone.md`、`manual/weak-network.md` | A、B、D |
