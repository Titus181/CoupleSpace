...
- G3／W3 已於 2026-08-11 通過。W1 驗證過的 Supabase Sign in with Apple session 已接入正式 App composition：啟動先等待本機 session 恢復，未登入才顯示 Apple 登入；取消與失敗可明確重試，登出需使用者確認。切片 8 將一般登出明確收斂為 `.local`：SDK 先刪除 local session，App 不因 remote logout error 恢復舊 authenticated UI，並在等待可能已在途的 refresh 後做最後 local cleanup。App 不另建 client-side 帳號資料列，同一 Apple／Supabase identity 沿用同一 user UUID；帳號設定只顯示 UUID 前 8 碼供同帳號複驗。真機已通過取消、登入、登出、強制結束後 session 恢復、離線入口停用與提示，以及同一 Apple 帳號登出再登入後識別碼不變；Apple 登入按鈕維持 50pt 高度。iPhone target build、7 個 affected tests、Harness 與 diff check 均通過。
...
- G11／W12 的九個本機功能切片已實作。共同時間線以裝置目前 calendar 將已載入 Moment 分成月份區段，月份與同月內容均維持新到舊，時間相同時以 Moment UUID 穩定排序；「快速跳轉」可定位目前已載入且符合篩選的月份，或定位日期當天最新一筆 Moment，不假裝能跳至尚未分頁取得的歷史。時間線可在「全部、心情、文字、照片、我們的一題」間單選篩選已載入 Moment；月份與日期目的地隨結果更新，空結果仍保留篩選器與載入舊頁能力。「我們」新增時間線／照片切換；照片頁只重用已成功同步的照片 Moment，以三欄網格按月份及照片舊到新排列，首次定位底部最新照片，詳情顯示建立者、建立時間，且只有具既有 `MomentSource` 時才提供返回主對話或約定討論。Moment 遠端歷史改採每頁 50 筆的 `(created_at, client_id)` 複合游標，server 以相同欄位降冪排序並多取一筆判定 `hasMore`，回應與問答只載入當頁 Moment；載入舊頁及 Realtime 首頁重讀都以 client UUID 合併去重，已載入的舊歷史不會被首頁刷新丟棄。主對話與約定專屬討論共用的聊天 service／model 同樣採每頁 50 則的 `(created_at, client_id)` 複合游標，reaction 與收藏來源只查當頁訊息；Realtime 首頁重讀保留已載入舊頁，Moment 來源不在本機時會逐頁尋找直到找到、失敗或沒有更多。時間線、照片頁與聊天均提供載入更早內容。遠端刷新及載入舊頁不再逐張下載 Moment 照片；Today 最新卡、時間線 LazyVStack 卡片與照片 LazyVGrid cell 出現時才依 Moment 識別碼載入，並以既有記憶體／本機快取避免重複下載。W11 既有共同日程已確認可承接 W12 找回流程：過期 scheduled 與 cancelled 約定依開始時間新到舊保留於「過往與已取消」並可返回專屬討論，前者仍可傳送，後者只讀。最小規則式每週回顧依裝置本地日曆取今天與前六日、排除未來時間，只以已載入 Moment 即時計算總數、類型計數及新到舊卡片；舊頁載入後就地重算，不建立第二份摘要，且明示不評分、不比較，零筆時不催補進度。固定三月份、Moment 55 筆、聊天 55 則分頁、過往約定及每週回顧 UI fixture、完整 `CoupleSpaceTests`、月份／日期跳轉、內容篩選、照片來源、Moment／聊天載入舊頁、聊天照片互動、來源返回、過往／已取消討論及每週回顧 UI regressions 已通過；其後依 roadmap 記錄完成完整 iPhone Gate 與兩支真實 iPhone 驗收，G11／W12 已完成。
- G12／W13 已於 2026-08-20 完成。Slice 8 候選以 rendered server identity 限制已讀前進範圍，並以 lifecycle generation 封住 stop／logout／account switch／relationship switch／closing 後的 stale async result。migration 040 在 `shared_appointments` 保存 server-owned `interaction_boundary_source_identity`：建立使用 appointment client ID，有效修改／取消使用已 applied operation ID；client 只能回傳畫面實際呈現的值，RPC 仍重新驗證 relationship、appointment、operation 與 ledger event，舊 snapshot 不得跳到 scope-latest。background appointment refresh 在寫回 cache／observer／reminder 前重驗 Auth user 與 active relationship，reminder activation 另使用 generation compare-and-set，讓 terminal cleanup 勝過在途工作。最後行為變更後的本機 Gate C 已通過：乾淨重放 migrations 001–040、31 files／533 pgTAP、schema lint、APNs 7／7、完整 iPhone 236 個測試定義／239 次 executions（0 failure、0 skip）、Auth scope guard、Harness v0.2.1 與 diff hygiene。linked test target `CoupleSpace-W1-Dev` 的 local／remote migration history 已同為 001–040；三台同候選 Simulator 的 production-auth 5C 文字 preflight 已確認停留「今天」／「我們」時未讀保持、背景／前景後仍保留，且只在真正開啟 main conversation 後清除。同一最終候選的兩支真實 iPhone 已通過同帳號 local logout、兩位 relationship owner、LOCK／PUSH／5C／W8、約定／討論／提醒、W9／W10／W11 active regression 與 lifecycle 整合。真機型號、iOS、APNs environment 均為 `未記錄`；本結論不包含 TestFlight Gate D、DR、UPGRADE 或 production APNs。
- W14-01 已於 2026-08-20 接受[資料生命週期與基本匯出契約](architecture/03-w14-data-lifecycle-contract.md)：共同 relationship 以 `active → closing → archived` 收斂，每位 owner 的個人封存再獨立進入 `archive_deleted`；帳號刪除、完整保留矩陣、export v1、audit／tombstone 與 A／B／C 隔離 fixture 已定案。這只是文件契約，不代表 migration、runtime、匯出、帳號刪除或 G13 已完成；W13 的 001–040／533 pgTAP／239 次 executions、三 Simulator 與兩真機證據只作 W14 基線。
...
已讀 acknowledgement 同樣不能以「目前 scope 最新事件」取代畫面證據：主對話與約定討論的文字／完成照片由 migration 039 以 visible message identity 映射 server ledger boundary；約定建立／有效修改／取消由 migration 040 以 snapshot 內的 server-owned source identity 映射。兩者都由 server 驗證 scope、來源與順序，只允許 read cursor 單調前進；缺少舊 ledger source 時安全 no-op，跨 scope 或偽造來源中性拒絕。這使先前畫面發出的延遲 read 即使晚於新事件完成，也不能清除使用者尚未看到的 activity。

各 authenticated content model 的 start／refresh／observe／drain／reminder side effect 必須綁定 lifecycle generation；`stop` 會先失效 generation，再等待已在途工作收斂後清除 cache、observer 與本機提醒。background push fetch 完成後也必須重新驗證同一 Auth user、同一 active relationship 與未變更的 reminder generation，不能只憑收到的 routing metadata 或舊 scheduler reference 寫回狀態。
...
- Auth session 與產品資料生命週期分離：依 PD-044，正式產品只保留目前裝置的 `.local` 登出；同帳號可在多台裝置使用同一份 server SSOT，但不提供 session／裝置 inventory、`.others`、`.global` 或指定 target 的遠端撤銷。登出不解除 relationship、不刪除共同內容或 owner-only 個人封存，也不以 `push_devices` 作授權證據；G12 已由同一最終候選的兩支真實 iPhone 驗證多裝置一致性、本機登出隔離及重新登入恢復。
- 目前通知固定只帶 allowlisted routing metadata、泛化文案與權威 badge，不查詢或傳送訊息正文、sender display name、照片或約定私密欄位；可選內容 preview 必須在 W13 完成後另案接受，不能沿用現有 gate。
...
- 匯出、刪除、帳號刪除與解除配對必須共享同一套[已核准的資料生命週期規則](architecture/03-w14-data-lifecycle-contract.md)；A 刪除自己的封存或帳號不得剝奪 B 的 owner-only 封存。
- Lifecycle audit、deletion tombstone 與 export integrity metadata 不得保存或複製名稱、私人稱呼、狀態、訊息、Moment、回答、約定正文、Emoji、媒體內容、token 或其他私人正文；災難復原必須先重播 tombstone 再重新開放資料。
...
5. 刪除、帳號刪除、匯出與解除配對依同一份 accepted 契約執行；shared relationship 與每位 owner archive 的終態不得混用。
6. iPhone 核心流程不依賴 Watch 或非 MVP 平台。
7. 未通過 G1 實測的外部技術選擇維持 provisional，不寫成既定事實。
8. 已成功同步的完整共同歷史必須能在遺失原裝置後由新裝置恢復；本機待送內容必須誠實顯示尚未備份。

其中第 1–5 項由 Supabase constraint、RLS、RPC、私有 Storage 與受控 Edge Function 執行伺服器端 enforcement；後續 W14 切片只能依已接受的資料矩陣建立 schema 與 runtime，並轉成最低可靠層的 unit／integration／Simulator regression；集中雙真機複驗只留在 W14 最後整合切片。
