...
- G11／W12 已開始第一個本機切片。共同時間線以裝置目前 calendar 將已載入 Moment 分成月份區段，月份與同月內容均維持新到舊，時間相同時以 Moment UUID 穩定排序；「跳到月份」只定位目前已下載的區段，不假裝能跳至尚未分頁取得的歷史。固定三月份 UI fixture、月份分組 unit 與既有建立 Moment 後進入「我們」的 UI regression 已通過。照片三欄網格、日期級跳轉、內容類型篩選、穩定游標分頁、漸進媒體、過往約定與規則式每週回顧尚未實作，因此 G11 不得視為完成。
