...
- G11／W12 已完成前六個本機切片。共同時間線以裝置目前 calendar 將已載入 Moment 分成月份區段，月份與同月內容均維持新到舊，時間相同時以 Moment UUID 穩定排序；「快速跳轉」可定位目前已載入且符合篩選的月份，或定位日期當天最新一筆 Moment，不假裝能跳至尚未分頁取得的歷史。時間線可在「全部、心情、文字、照片、我們的一題」間單選篩選已載入 Moment；月份與日期目的地隨結果更新，空結果仍保留篩選器與載入舊頁能力。「我們」新增時間線／照片切換；照片頁只重用已成功同步的照片 Moment，以三欄網格按月份及照片舊到新排列，首次定位底部最新照片，詳情顯示建立者、建立時間，且只有具既有 `MomentSource` 時才提供返回主對話或約定討論。Moment 遠端歷史改採每頁 50 筆的 `(created_at, client_id)` 複合游標，server 以相同欄位降冪排序並多取一筆判定 `hasMore`，回應與問答只載入當頁 Moment；載入舊頁及 Realtime 首頁重讀都以 client UUID 合併去重，已載入的舊歷史不會被首頁刷新丟棄。時間線底部與照片頁頂部均提供載入更早內容。遠端刷新及載入舊頁不再逐張下載照片；Today 最新卡、時間線 LazyVStack 卡片與照片 LazyVGrid cell 出現時才依 Moment 識別碼載入，並以既有記憶體／本機快取避免重複下載。固定三月份與 55 筆分頁 UI fixture、完整 `CoupleSpaceTests`、月份／日期跳轉、內容篩選、照片來源及載入舊頁 UI regressions 已通過。聊天穩定游標分頁、過往約定與規則式每週回顧尚未實作，因此 G11 不得視為完成。
