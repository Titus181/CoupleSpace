...
- 登入或回到前景後的 recovery coordinator 會立即嘗試目前 active relationship 的待送 queue；失敗且 queue 仍存在時，依 1 秒、4 秒短退避再試，合計最多三次，之後保留 Outbox。真機 A＋Simulator B 已通過短暫斷線期間自動送達、三次失敗後保留待送項目，以及恢復網路後由下一次前景事件送達且不重複。這只是 W1 前景候選，不提供網路監聽、背景傳送或無限重試保證。
