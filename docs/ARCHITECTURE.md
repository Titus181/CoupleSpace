...
- `CoupleSpaceTests/` 已涵蓋 W2 runtime environment、核心分頁順序／預設入口，以及 W1 設定、登入 session、nonce、冪等／FIFO outbox、排序、照片政策、解除配對與通知隱私等純規則；`CoupleSpaceUITests/` 已加入啟動後預設進入「今天」並可切換三分頁的核心骨架旅程。
