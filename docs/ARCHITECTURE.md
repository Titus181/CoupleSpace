...
- G4／W4 於 2026-08-11 開始開發。正式 App composition 已在登入後加入 relationship gate：未配對者可建立並以系統分享表分享一小時邀請碼、貼上並接受或明確拒絕邀請；邀請失效或拒絕後由原邀請者重試時輪替 token，成功後才進入三分頁，雙方可在帳號設定核對相同 relationship UUID 前 8 碼。切換 Supabase identity 時會先清除上一個 pairing presentation state 並重新經 RLS 讀取，配對前仍保留登出。這一 slice 的 iPhone build、完整 75-case suite、最後 affected unit／UI regression，以及重建本機 Supabase 後包含 migration 014 的 147 個 pgTAP 均已通過；migration 014 已部署 Supabase 測試專案，遠端 migration list 一致且 linked schema lint 無錯誤。兩支真實 iPhone 的邀請、拒絕、失效與重試仍是 G4 完成 gate，因此 G4 尚未標示完成。
