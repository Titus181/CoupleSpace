...
- G11／W12 已完成前兩個本機切片。共同時間線以裝置目前 calendar 將已載入 Moment 分成月份區段，月份與同月內容均維持新到舊，時間相同時以 Moment UUID 穩定排序；「跳到月份」只定位目前已下載的區段，不假裝能跳至尚未分頁取得的歷史。「我們」新增時間線／照片切換；照片頁只重用已成功同步的照片 Moment，以三欄網格按月份及照片舊到新排列，首次定位底部最新照片，詳情顯示建立者、建立時間，且只有具既有 `MomentSource` 時才提供返回主對話或約定討論。詳情使用 sheet，因此關閉後保留原網格位置。固定三月份 UI fixture、完整 `CoupleSpaceTests`、月份跳轉與照片來源 UI regressions 已通過。日期級跳轉、內容類型篩選、穩定游標分頁、可見範圍媒體下載、過往約定與規則式每週回顧尚未實作，因此 G11 不得視為完成。
