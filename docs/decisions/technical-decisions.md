...
- outbox 的正式自動排程、長時間退避、網路監聽、背景執行與正式訊息長佇列上限；W1 只保留「登入或回到前景後，active relationship 立即嘗試，失敗後以 1 秒、4 秒延遲再試，合計最多三次」的可撤換候選。此候選已通過真機 A＋Simulator B 的短暫斷線、上限停止保留及下一次前景恢復送達實測。
