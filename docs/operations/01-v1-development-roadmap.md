...
前景 outbox recovery 已進一步加入有限次短退避：登入／回到前景時立即嘗試，若仍有同一 active relationship 的待送項目，依序等待 1 秒與 4 秒再試，合計最多三次；之後停止並保留 Outbox。這不包含網路監聽、背景 task 或無限輪詢。iPhone 17 Pro Simulator 的完整 `CoupleSpaceTests` 56／56 通過；真機 A＋Simulator B 亦已完成短暫斷線後不按人工重試即自動送達、持續斷線至三次耗盡後保留待送項目，以及恢復網路後由下一次前景事件送達且對方只收到一次的驗證。

以下實作細節尚未決定，不得在規劃中默認某一實作方式：

1. 正式訊息資料模型、長佇列真機壓力、正式自動排程、長時間退避、網路監聽與背景重試；message／marker 各 100 筆的本機 FIFO regression，以及登入／前景恢復的一次性 immediate retry 與三次有限短退避真機流程均已通過。正式長時間重試與背景執行仍未定案。
...
若 TestFlight build 提供「支持開發者」流程，只能使用 StoreKit／Sandbox 驗證，不向測試者實際收款。G17 後若決定在 App Store 正式供應版本啟用，必須與訂閱及其他 StoreKit 工作共同排序，並完成當時適用的商品價格、審查說明、交易、取消、失敗與退款驗證。
