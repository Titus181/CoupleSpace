...
| W12，10/19–10/25 | **G11「我們」與基本回顧** | 共同時間線不只依序無限堆疊，須按月份分組並支援日期／月份快速跳轉及最小內容類型篩選；「我們 → 照片」以三欄網格只瀏覽照片 Moment，月份由上到下從舊到新，首次進入定位底部最新內容、向上載入更早月份，詳情可顯示來源並返回正確對話／約定且保留原捲動位置；聊天與 Moment 的長期歷史使用穩定游標分頁，媒體依可見範圍漸進載入；使用正確顯示名稱／私人稱呼標示參與者與建立時間；可查看過往約定並返回相關討論；基本每週回顧採規則式彙整，不使用 AI |
