...
| 階段 | 建議時機 | 推薦範圍 | 進入下一階段的證據 |
|---|---|---|---|
| 第一階段：Moment 照片瀏覽 | W12／G11 | 「我們 → 照片」、照片 Moment 三欄網格、舊上新下、預設定位底部最新內容、向上載入舊月份、詳情來源與返回定位；復用既有 Moment／Storage／來源 identity，不新增相簿資料模型 | 兩支真實 iPhone 的月份／順序一致，分頁不遺失或重複，直／橫／方形照片顯示正確，來源返回與離線快取行為可驗證 |
| 第二階段：全部共同照片候選 | G16 可用性觀察，G18 決定是否排入下一版 | 視證據加入「Moment／全部照片」切換，聚合主對話及約定討論的已同步照片，並由使用者主動留成 Moment；不自動收藏、不建立第二套媒體管線 | 使用者反覆回報找不到未收藏照片，能理解一般照片與 Moment 的差異，來源返回與收藏流程提升找回成功且未造成備份誤解；Storage／egress、失敗率與單人營運成本可接受 |
| 第三階段：整理與回憶產品 | 第二階段證明需求後 | 多本相簿、封面、自訂排序、進階搜尋、回憶編排與精美匯出；再決定哪些進階整理屬 Plus。短影片維持獨立可靠性 gate | 出現反覆整理行為、回顧／留存價值與付費意願，而不是只有一次性試用或更高媒體用量 |

第一階段是 W12 已接受範圍；第二、三階段是推薦順序，不是承諾版本。基本照片瀏覽、既有內容存取、來源返回、換機恢復及既有資料匯出不得因後續 Plus 規劃被撤回或鎖住。完整產品邊界見 [PD-035](../decisions/product-decisions.md#pd-035共同照片先作-moment-瀏覽層再依證據擴張)。

### G4A／G4B 配對邀請體驗插入順序（2026-08-15）

- **G4A 已完成：** 在目前 G10 的第二輪雙機驗收前，先完成八位短碼的伺服器產生／輪替／唯一性、authenticated user 無效嘗試限流、client 正規化與舊 token 相容。短碼仍沿用一小時、單次使用、拒絕／取消／過期／接受後失效及一人一個 active membership；分析、log、推播與 release record 不保存完整碼。此切片只降低目前頻繁解除／重新配對的操作成本，不改變已完成 G4 的授權不變量，也不以 UI 美化延後安全與回歸測試。
- **G4A 自動化 gate：** 新增最低層可重現的正規化、碰撞重試、舊 token 相容、單次使用、輪替、拒絕、取消、過期、限流、同時邀請與第三人隔離測試；通過 affected iPhone tests、配對 UI regressions、完整本機 database suite、schema lint、iPhone build、Harness 與 diff hygiene。linked Supabase migration 仍是獨立人工核准 gate。
- **G4A 真機 gate：** 兩支真實 iPhone／兩個 Apple 身分至少五輪完成「解除到允許重新配對的狀態 → 建立新邀請 → 輸入不同格式短碼或完整分享訊息 → 雙方讀回同一 relationship」，每輪舊短碼不可接受；另驗證弱網、App 重啟、拒絕、取消、過期及兩邊同時建立邀請的既有恢復路徑。完成後才以新的配對基線執行 G10 最終雙機複驗。
- **G4B TestFlight 前完成：** Universal Link、未安裝落地頁、iOS 系統分享表的 URL／品牌預覽／備援短碼、三種固定邀請語氣、有效邀請後的名稱確認，以及「光窗交會」成功動畫納入 G15 候選版。動畫只在伺服器確認 `2/2` relationship 後播放，支援減少動態效果，不把 Instagram 公開限時動態當作伴侶配對入口。完整規格見[伴侶邀請、短配對碼與配對儀式](../design/03-partner-invitation-and-pairing.md)。

### G4A 開發證據（2026-08-17）

- migration 032 已在本機空資料庫加入與 UUID token 分離的八位短碼、碰撞重試、現有 token 相容，以及 authenticated user 十分鐘最多五次無效識別嘗試；舊 UUID RPC 保留供既有 App 使用，新 client 使用 v2 RPC。2026-08-17 已依 backend-first 順序部署 linked Supabase，遠端 migration 001–032 一致且 linked schema lint 無錯誤。
...
- 第四個本機切片移除 `refresh`／載入舊頁後逐張下載照片的行為；Today 最新卡、時間線 LazyVStack 與共同照片 LazyVGrid 只在項目出現時要求照片資料，同一 Moment 的已載入資料與既有本機快取直接重用。Unit 驗證刷新後不可見照片零下載、可見照片重複請求只下載一次；完整 `CoupleSpaceTests` bundle 及月份跳轉、照片詳情／來源返回、舊頁載入三個 W12 UI regressions 均通過。本切片未新增背景預取、圖片淘汰或第二套快取。
- 第五個本機切片在共同時間線加入單選內容類型 Menu，直接對已載入 Moment 提供全部、心情、文字、照片與「我們的一題」五種選項；月份跳轉只列出篩選後仍有內容的月份，空結果保留篩選控制與載入更早頁入口。Unit 覆蓋四種內容分類，UI regression 覆蓋文字隱藏、照片結果及無題目空狀態；既有月份跳轉 regression 同時通過。本切片不新增全文搜尋、多選條件或 server-side filter。
- 第六個本機切片將月份入口收斂為單一「快速跳轉」Menu，分列目前已載入且符合內容篩選的月份與日期；月份定位區段標題，日期定位同日最新一筆 Moment。Unit 覆蓋同日多筆只保留最新目的地及日期新到舊；月份、日期與內容篩選三個 UI regressions 同時通過。本切片不查詢未下載歷史，也不新增日期搜尋索引或 server RPC。
- 第七個本機切片為主對話與約定專屬討論共用的聊天 service／model 加入每頁 50 則的 `(created_at, client_id)` 複合游標；Supabase 以相同欄位降冪、多取一則判斷舊頁，reaction 與收藏來源只查當頁訊息，UI 維持舊到新並在頂部載入更早內容。Realtime 最新頁重讀以 UUID 合併保留舊頁；由 Moment 返回尚未載入的來源時會逐頁尋找，分頁失敗即停止。Unit 覆蓋 55 則合併、Realtime 保留與相同 timestamp 跨頁不漏訊息；聊天分頁、照片互動及來源返回 UI regressions 通過。本切片不新增 schema 或 migration。
- 第八個本機切片確認 W11 已存在的過往約定入口可直接承接 W12 找回流程，不另建第二套歷史資料：scheduled 且已過開始時間的約定與 cancelled 約定都保留在「過往與已取消」，依開始時間新到舊排列並可返回各自專屬討論；前者保留輸入列，後者仍可閱讀既有內容但維持只讀。固定近期／過往／已取消 fixture、分類排序 unit 與詳情／討論 UI regression 通過。本切片只補明確驗證證據，不新增 schema、migration 或產品行為。
...
- W12 功能範圍已由九個本機切片實作；後續兩支真實 iPhone 驗收已完成。以隔離 fixture 驗證 2026 年 6、7、8 月的心情、文字、照片與「我們的一題」Moment：時間線的月份／日期快速跳轉、類型篩選與舊頁載入均維持完整、去重及穩定順序；共同照片的月份順序、三欄版面、來源詳情與返回定位正常；過往約定可返回討論。聊天向上滑動會自動載入舊頁，無額外按鈕；每週回顧在兩機的七天範圍、總數、類型計數、分頁後去重與無 Moment 空狀態均正常。G11／W12 正式完成。

### G12／W13 完成證據（2026-08-20）
...
- 已加入 Face ID usage description、App Lock lifecycle／取消／失敗／無法驗證仍保留遮蔽、啟用／停用與偏好恢復的 unit，以及未啟用啟動與已儲存偏好遮蔽的 UI regression。iPhone Simulator build、Harness 與 diff hygiene 通過；2026-08-18 已依 `LOCK-001` 由真實 iPhone 確認 Face ID／裝置密碼、冷啟動、背景返回、App switcher snapshot、鎖屏檢查，以及 session／relationship／Outbox 不變。此切片完成時同帳號多裝置與 production APNs 隱私尚未完成整合驗收，G12 尚未關閉。
- 第二個切片「關係互動未讀、約定 lifecycle 與 App Badge」已完成：伺服器以穩定互動 identity 記錄對方的主對話／約定討論文字與照片，以及共同約定的建立、有效修改與取消；同一 operation 重送不重複增加未讀或推播。對話分頁、近期約定與 App icon 讀取同一份 relationship authoritative 未讀數，進入主對話、指定約定詳情或討論後才標記對應 scope 已查看。推播帶入權威 badge，並固定使用泛化、不含私密內容的文案。先前提前加入的文字內容 preview runtime 已在切片 8 候選版前停用：設定入口、client preference 寫入、Function 正文／sender 查詢與 APNs preview payload 均不屬於目前契約；migration 038 移除三參數 registration overload、重設 migration 037 的相容欄位，runtime 不再讀取或寫入該偏好。任何可選內容 preview 必須等 W13 完成後另開工作項目、重新接受隱私契約及 gate。空資料庫 migration、pgTAP、APNs Function regression、Simulator regression 與兩支真實 iPhone 已確認文字、照片、建立／修改／取消、前景／背景／終止／鎖定、重送去重、讀取收斂與 badge 一致。此證據只關閉第二個切片；當時本機提醒 W13 回歸及 session capability／撤銷尚未完成，G12 保持進行中。
- 切片 7 已完成 Supabase Auth capability record、無私密資料的 SDK source／remote metadata probe 與受控三 Simulator preflight：`supabase-swift 2.54.1` 只有 `local`／`global`／`others` sign-out，沒有 session inventory 或指定 session revoke；測試 JWT 也沒有 optional `session_id`。A／B／C 的歷史 probe 曾觀察 `.others` 後 A、C 不受影響、B 於 proactive refresh 時回到登入，且 relationship／共同資料／兩份 owner archive 不變。這只使 `SESSION-CAPABILITY-PREFLIGHT` 達到 PASS，證明 SDK API surface 與 no-inventory 邊界；不支持 shipping `.others`，也不建立 registry、migration、Edge Function、service-role secret 或 production Auth policy。
- 切片 8 的兩支真實 iPhone `SESSION-001` 中，A 送出 `.others` 後 B 仍可用既有 JWT 讀取；B manual refresh 隨後成功，Access JWT expiry 更新為 9:44 PM，Auth 與基線資料保留。其他未提供的時間、HTTP／audit 與 session identity 欄位均記為 `未記錄`。此結果是 PD-043 all-others 契約的 FAIL，不能寫成 JWT 殘留窗口。PD-044 因此取代 PD-043：候選版移除「登出其他所有登入」與所有 inventory／remote-revoke product wiring，只保留穩定化 `.local` 登出及同帳號多裝置 server SSOT。`SESSION-001` 保留為 retired stable ID，候選版記 `NOT_APPLICABLE (REMOVED)` 而非 PASS；既有 30 files／481 pgTAP、generic APNs 7／7、iPhone scheme 214／214、preflight／Harness／diff hygiene 是移除前的歷史本機證據，不能單獨關閉最後行為變更後的 `W13-INTEGRATION-001`。因此在該階段 G12／W13 仍為 PENDING，剩餘 gate 是兩支真機 local logout／重新登入恢復、LOCK、PUSH、activity-badge、reminder、lifecycle 與 W8–W11。
- 5C 雙真機驗收發現 `REG-014`：B 停留「今天／我們」時，A 傳文字或照片後未讀先增加、隨即被誤清。候選修正將 main conversation 可見性綁定 active scene、實際呈現、未鎖定與非 appointment source routing，拒絕離開畫面後舊 read／badge response 回寫，並以 migration 039 的 message-bound cursor 取代 scope-latest main／discussion read；同時撤銷 internal event recorder 對 client roles 的 execute 權限。最後本機 gate 於 iPhone 17 Pro Simulator（iOS 26.5）通過 fresh migration 001–039、30 files／492 pgTAP、generic APNs 7／7、211 test definitions／214 executions（0 failure、0 skip）、session scope guard、Harness v0.2.1 與 diff hygiene。migration 038／039 已依序部署至 linked test project，遠端 migration history 確認 local／remote 001–039 一致；遠端 schema dump 確認 push registration 只剩兩參數版本且 bounded read RPC／`clock_timestamp()`／internal helper 權限收斂已落地，兩條匿名 PostgREST smoke request 均以 `401 / 42501` 被授權邊界拒絕，沒有 missing-function 或 overload-ambiguous 錯誤。CLI 套用後的 migration catalog 快取曾因 `pg-delta` 2.5 秒連線逾時輸出警告，但上述獨立遠端證據確認交易已完成，未重送 migration。此階段兩台候選 build 的完整 5C、App icon badge 與後續 reminder／W8–W11／lifecycle 真機 gate 仍為 PENDING；此本機與部署證據當時不得關閉 G12／W13。

- 其後 lifecycle audit 再揭露 `REG-015`／`REG-016`：model stop 或 Auth／relationship 終止後，在途 refresh／observer／drain 可能復活舊 cache、observer 或提醒；background appointment refresh 也可能在 logout、account／relationship switch 或 closing cleanup 後以舊 context 重新排程。候選版為 authenticated content models 加入 lifecycle generation 與 join，background 路徑完成 fetch 後重驗 Auth user／active relationship，reminder scheduler 以 generation conditional activation 讓 terminal cleanup 勝出。約定 lifecycle 已讀另加入 migration 040：server 隨 appointment snapshot 保存建立 client ID 或 applied update／cancel operation ID 作 `interaction_boundary_source_identity`，RPC 重新驗證後只前進到實際呈現的 boundary；舊 snapshot 不得清除較晚 lifecycle event。2026-08-20 已只將原先 pending 的 040 部署至 linked test target `CoupleSpace-W1-Dev`；部署後 local／remote migration history 同為 001–040，第二次 dry-run 回報 remote up to date。linked schema lint exit 0（僅 migration 034 既有 unused `recipient_id` warning），遠端 schema／ACL 與匿名授權 smoke 均通過。
- 最後行為變更後的本機證據：iPhone 17 Simulator（iOS 26.5）的 focused unit `/private/tmp/CoupleSpace-slice8-unit-lock-boundary-final.xcresult` 為 191／191 passed、0 failure、0 skip，focused UI `/private/tmp/CoupleSpace-slice8-appointment-ui-focused-2.xcresult` 為 1／1 passed。最終 `quality/scripts/run-full-automated-suite.sh --reset-local-database` 亦以 exit 0 完成：乾淨重放 migrations 001–040、31 files／533 pgTAP、local schema lint（僅 migration 034 既有 unused `recipient_id` warning）、generic APNs 7／7、iPhone 17 Pro Simulator（iOS 26.5）完整 scheme 236 個測試定義／239 次 executions（0 failure、0 skip）、session scope guard、Harness v0.2.1 與 diff hygiene；artifact 為 `/tmp/CoupleSpace-release-evidence.l8E2mW/CoupleSpace.xcresult`。
- 三台 Simulator（iPhone 17／17 Pro／17 Pro Max）已安裝同一候選 App（hash prefix `b05d650a`），保留既有 Auth 且皆顯示兩位 owner 的 active relationship。production-auth 5C 文字 preflight 在接收端停留「今天」與「我們」兩條路徑均確認正數未讀至少維持 10 秒、背景／前景後仍保留；真正打開 main conversation 後才清 0，同帳號另一台亦在前景同步後清 0。artifacts 為 `/private/tmp/CoupleSpace-w13-prod-hidden-today.xcresult`、`/private/tmp/CoupleSpace-w13-prod-hidden-us.xcresult`、`/private/tmp/CoupleSpace-w13-prod-open-clear-2.xcresult`、`/private/tmp/CoupleSpace-w13-prod-same-account-clear.xcresult`。這些只作 Simulator preflight；其後同一最終候選已由兩支真實 iPhone 完成並通過六段整合 gate：同帳號 local logout、切回兩位 relationship owner、LOCK／PUSH／5C／W8、約定／討論／提醒、W9／W10／W11 active regression，以及 lifecycle。`W13-INTEGRATION-001` 為 **PASS**，G12／W13 正式完成；`SESSION-001` 仍只記 `NOT_APPLICABLE (REMOVED)`。真機型號、iOS 與 APNs environment 均為 `未記錄`；此結論不宣稱 TestFlight Gate D、DR、UPGRADE 或 production APNs 已完成。
...
- 建立流程現已在遠端 RPC 前寫入 user＋relationship scoped 的持久 Appointment Outbox；失敗項目以待同步／可重試狀態保留，前景及網路恢復沿用 stable client UUID 重送，包含遠端成功但本機 acknowledgement 遺失時不重複建立。最近 200 筆已同步約定另保存為相同 scope 的裝置唯讀顯示快照；離線冷啟動會先恢復快照，再依 FIFO 疊加待送修改／取消，恢復網路後仍由 Supabase RLS 結果校正。成功登出、遠端確認 relationship 失效或 identity 改變時清除快照。聊天室輸入列 `＋` 與長按已同步文字訊息會開啟同一表單；長按只預填原文並保留來源 ID，不分析日期或自動建立。
...
- 每筆已同步約定現可由詳情進入 appointment UUID scoped 的專屬討論；資料仍寫入既有 `shared_items`、沿用相同訊息／Emoji／Realtime 管線，不建立第二套訊息資料。此切片提供文字、照片、Emoji、server timestamp、獨立未讀游標，以及與主對話／其他約定隔離的持久混合 FIFO Outbox 與 snapshot；stable message UUID 重試不重複或跨 scope 移動。照片復用 W1 私有 bucket、relationship-wide quota、W10 壓縮／預覽／反應與 lifecycle reconciliation，不建立第二套媒體管線。已取消約定仍可閱讀原討論但不顯示輸入列，server 也拒絕新文字或照片。「對話」頂部現提供最多五筆依訊息／照片／Reaction／重大狀態紀錄最新活動排序的近期約定討論入口，只顯示約定標題、活動時間與目前使用者未讀數，不回傳內容摘要；點擊會直接進入同一個專屬討論，分頁 badge 合併主對話與約定討論未讀。已同步的討論文字與照片現可沿用 W10 收藏 RPC 建立 Moment，Moment 持久保留 appointment UUID；收藏成功後會主動刷新共同時間線，收藏標記可由遠端或本機快照恢復。從「今天／我們」返回來源時，主聊天與約定討論 route 互斥，且同一約定的不同 message UUID 會重新定位並標示正確訊息。每台 iPhone 現會在使用者明確允許後，為已同步且仍 scheduled 的未來約定排一個本機提醒；固定 appointment UUID 會在重啟、改期與 Realtime 重讀時覆寫同一請求，取消、登出或解除配對會移除舊請求。鎖定畫面只顯示通用文字，不含標題、地點與註記；點擊會先刷新再開啟同一 appointment UUID 詳情。
- 解除配對封存現會為兩位 owner 各自複製約定快照、專屬討論的 appointment scope、原始建立者與不可偽造的重大事件；約定來源訊息及討論文字／照片都以 archive-local 外鍵保留關聯，seal 重試不重複，刪除其中一份封存只級聯刪除該 owner 的 W11 快照。migration 同時回填既有個人封存，RLS 只允許 owner 讀取，伴侶與第三人均不可讀取另一份封存。完整 manifest 匯出與其他衍生資料的生命週期仍由 W14 統一處理，不以本切片提前宣稱完成。
- Debug 技術驗證工具現會以 owner RLS 讀取自己的封存並顯示約定、專屬討論項目與重大事件數量；純本機 audit 會核對每則討論與事件的 appointment UUID、約定來源訊息及原建立者，完整時明示「約定封存關聯完整」，任一斷鏈則顯示異常數。這只提供 W11 真機驗收證據，不是 W14 的正式封存瀏覽介面。
- migrations 001–031 已由空本機資料庫套用，27 份 pgTAP 共 441 項與 schema lint 全數通過；封存 audit 的測試建置與 focused runtime 1/1 通過。依正式 Gate 關閉平行 runner 後，先前完整 iPhone unit 129/129 通過、0 失敗、0 跳過；先前非標準平行執行曾使既有狀態到期時序 case 超過觀察窗，未修改 assertion 或以重跑取代正式 Gate。migrations 023–031 已部署 linked Supabase；獨立 remote migration history 確認 local／remote 001–031 一致，部署後 dry-run 回報 up-to-date，linked `extensions`／`public` schema lint 無錯誤。首次 push 已明確依序套用 023–031，但 CLI 停在收尾；確認遠端交易完成後才停止殘留程序，未重送 migration。首輪 W11 雙機驗收的通用提醒與通知來源跳轉通過；弱網改期後離線強制結束並重開先揭露已同步約定沒有本機顯示基底，加入 scoped 快照後第二次實測可恢復約定，但待送改期仍因遠端刷新／Realtime 等待而暫時顯示舊時間。修正現於任何遠端等待前先把本機 operation FIFO 疊加到快照，並以懸停遠端請求的 focused regression 證明尚未傳送時已顯示新時間與待送提示；最終測試建置、focused runtime 2/2 與正式串行完整 iPhone unit 131/131 通過、0 失敗、0 跳過。第二輪第三次雙機驗收已確認離線重開立即保留新時間與待同步提示，恢復網路後兩台收斂；另發現離線提示壓到約定頁導覽控制，現已改為只占主分頁版面的固定列，focused UI regression 證明進入約定編輯後提示不可互動且取消按鈕可操作，兩支真實 iPhone 安裝修正版後亦確認無遮擋。第三輪提醒驗收確認強制結束並重開兩次後只收到一次通用通知、點擊開啟正確約定，伴侶取消後原提醒不再出現。第四輪解除配對封存驗收中，A、B 建立封存後皆顯示共同約定 12、專屬討論項目 5、重大事件 16 與「約定封存關聯完整」；刪除 A 封存後 A 歸零，B 的 12／5／16、完整 audit 與畫面均不受影響。四輪兩支真實 iPhone 驗收全部通過，G10／W11 正式完成。
...
2026-08-18 已通過。兩位使用者可從 Moment 延伸至自由聊天，也可從共同約定進入專屬討論；重要訊息可收藏為 Moment，並能在共同時間線找回內容及返回相關約定或對話。
