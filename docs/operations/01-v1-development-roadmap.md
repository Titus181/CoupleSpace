...
- 「我們」現可進入共同日程，分開顯示近期與過往／已取消約定，並可直接建立或開啟系統 graphical 基本月曆；月曆依使用者本地日期顯示當日 scheduled／cancelled 約定，並可進入同一筆詳情。已同步且仍 scheduled 的項目可編輯或取消，取消後保留原內容且不可由編輯復活。主對話以 appointment UUID 將約定卡片與一般訊息共同排序；已同步卡片可開啟同一筆詳情，編輯、取消及 Realtime 重讀會就地更新原卡片，不建立平行資料。編輯與取消現會先寫入 user＋relationship scoped 的持久 operation Outbox，再立即套用預期狀態並以單一 FIFO 傳送；離線、App 重啟或遠端成功但本機 acknowledgement 遺失時會沿用同一 operation UUID 恢復，取消後拒絕排入較晚的編輯。伺服器以 relationship-scoped operation receipt 真正辨認重送；舊 ack 即使晚於伴侶的新編輯抵達，也只能回讀目前狀態而不再次改寫。edit／cancel RPC 在同一 transaction 鎖住 relationship 與 appointment，避免解除配對交錯寫入；已取消、不存在或關係停止寫入等 terminal 結果會移出 queue 並繼續後續 FIFO，只有暫時性錯誤保留重試。日期／時間重大變更或取消的永久系統紀錄尚未納入。
- 每筆已同步約定現可由詳情進入 appointment UUID scoped 的專屬討論；資料仍寫入既有 `shared_items`、沿用相同訊息／Emoji／Realtime 管線，不建立第二套訊息資料。此切片提供文字、照片、Emoji、server timestamp、獨立未讀游標，以及與主對話／其他約定隔離的持久混合 FIFO Outbox 與 snapshot；stable message UUID 重試不重複或跨 scope 移動。照片復用 W1 私有 bucket、relationship-wide quota、W10 壓縮／預覽／反應與 lifecycle reconciliation，不建立第二套媒體管線。已取消約定仍可閱讀原討論但不顯示輸入列，server 也拒絕新文字或照片。「對話」頂部現提供最多五筆依訊息／照片／Reaction 最新活動排序的近期約定討論入口，只顯示約定標題、活動時間與目前使用者未讀數，不回傳內容摘要；點擊會直接進入同一個專屬討論，分頁 badge 合併主對話與約定討論未讀。已同步的討論文字與照片現可沿用 W10 收藏 RPC 建立 Moment，Moment 持久保留 appointment UUID；收藏成功後會主動刷新共同時間線，收藏標記可由遠端或本機快照恢復。從「今天／我們」返回來源時，主聊天與約定討論 route 互斥，且同一約定的不同 message UUID 會重新定位並標示正確訊息。migrations 001–029 已由空本機資料庫套用，26 份 pgTAP 共 405 項、123 項 iPhone unit、來源 route UI regression、測試建置與 schema lint 均取得 exit 0。通知、日期／時間重大變更或取消的永久系統紀錄，以及解除配對封存中的約定／討論關聯仍未完成，因此 G10 繼續維持進行中。
