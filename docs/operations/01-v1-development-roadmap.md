...
- migrations 001–031 已由空本機資料庫套用，27 份 pgTAP 共 441 項與 schema lint 全數通過；最新串行 iPhone unit 128 項、提醒 focused runtime 4 項與 `git diff --check` 的既有成功證據仍有效。G10 尚待兩支真實 iPhone 完成通知、弱網與解除配對封存的最終驗收，因此繼續維持進行中。
