...
![CoupleSpace W12 共同照片分階段概念](assets/product-concepts/shared-photos-w12-concept-v2.png)

這張圖只說明 W12 第一階段的導覽與時間方向，不是現有 App 截圖或像素級驗收基準。使用者由「我們 → 照片」進入，只瀏覽已成為 Moment 的照片；月份由上到下從舊到新，首次進入定位於底部最新內容，向上才載入更早月份。點開後可查看建立者、日期與來源，並在來源存在時返回原對話或共同約定脈絡；沒有來源時不顯示虛假的返回行動。

圖中的照片、對話、天數、角色呈現與文案均為生成式情境佔位，不是正式資料、品牌資產或核准 copy。第二階段的全部聊天／約定討論照片聚合與主動留成 Moment，以及第三階段的自訂相簿與進階整理，須依 [PD-035](../decisions/product-decisions.md#pd-035共同照片先作-moment-瀏覽層再依證據擴張) 的 gate 另行決定，不因出現在討論稿中而視為已承諾。
